...
class Recommender {
  constructor(firebaseRef) {
    ...
    this.userName = null;
    this.logger = new Logger(firebaseRef);
  ...
  getSeasonByTerm(term) {
    ...
    if (recommendations.length > 0) {
      return (
        `You can visit ${recommendations[0].name} during ` +
        recommendations[0].season.map(s => s.toLowerCase()).join(", ")
      );
    } else {
      recommendations = this.engine.attractionsArray.filter(attraction =>
        attraction.category.includes(term)
      );
    }
    if (recommendations.length > 0) {
      return (
  ...
  getMapByTerm(term) {
    ...
    if (recommendations.length > 0) {
      const rec = recommendations[0];
      return `https://maps.google.com/maps?q=${rec.latitude},${
        rec.longitude
      }&hl=en&z=9`;
    } else {
